...
export default function ManagePage(){
  ...
  const operational=[
    ...
    {href:"/attention",icon:<AlertTriangle/>,title:"Needs Attention",copy:"Human-action queue for holds, mismatches, returned approvals, reconciliation issues, and exceptions.",metric:`${earnings?.summary?.payment_exceptions||0} payment exceptions · ${earnings?.summary?.awaiting_approval||0} awaiting approval`},
  ...
  const administration=[
    {href:"/employees",icon:<Users/>,title:"Employees",copy:"Review compensation participation, access readiness, plan assignment, eligibility dates, and workflow readiness."},
    {href:"/plans",icon:<ClipboardCheck/>,title:"Plans",copy:"Manage effective-dated compensation plan versions, components, assignments, and payout timing."},
    {href:"/workflow",icon:<ShieldCheck/>,title:"Workflow",copy:"Review configured approval chains and Finance handoffs without hardcoding personnel."},
    {href:"/reconciliation",icon:<History/>,title:"Reconciliation",copy:"Compare source data, approved compensation, actual payments, and historical corrections."},
    {href:"/hubspot",icon:<RefreshCw/>,title:"HubSpot",copy:"Monitor source synchronization, associations, compensation-sensitive field changes, and refresh status."},
    {href:"/users",icon:<Settings/>,title:"Users & Permissions",copy:"Review app access, roles, drafts, compensation readiness, and the permission catalog."},
    {href:"/audit",icon:<History/>,title:"Audit Log",copy:"Inspect who changed what, when, why, and the before/after state for sensitive actions."},
  ];
  ...
  return <main style={{minHeight:"100vh",background:"#f5f7fa",padding:28,fontFamily:"Inter,Arial,sans-serif",color:"#051b34"}}><div style={{maxWidth:1250,margin:"0 auto"}}>
    <section aria-label="Management workspace header" style={{display:"flex",justifyContent:"space-between",gap:24,alignItems:"flex-start",marginBottom:28,flexWrap:"wrap",background:"transparent"}}><div style={{flex:"1 1 700px"}}><small style={{fontWeight:800,letterSpacing:1.2,color:"#2095f3"}}>ENGAGIFII COMPENSATION</small><h1 style={{fontSize:36,margin:"7px 0 8px"}}>Management workspace</h1><p style={{margin:0,color:"#647184",maxWidth:820,lineHeight:1.5}}>One operating model for compensation review, approval, Finance acceptance, payroll, administration, and audit. Eligible compensation can be submitted at any point in the month.</p><small style={{display:"block",marginTop:10,color:"#718096"}}>{access?.full_name||access?.email||"Signed-in user"}{isAdmin?" · System Administrator":""}</small></div><button onClick={load} disabled={loading} style={{display:"flex",alignItems:"center",gap:7,padding:"10px 13px",background:"white",border:"1px solid #d9e2eb",borderRadius:9,fontWeight:700}}><RefreshCw size={16}/>{loading?"Refreshing…":"Refresh"}</button></section>
    ...
    <section style={{background:"#eef7ff",border:"1px solid #cfe3f7",borderRadius:12,padding:14}}><b>Internal beta workspace</b><p style={{margin:"3px 0 0",color:"#4f6274"}}>The consolidated management routes and employee self-service workspace are now the authoritative application surfaces. This beta is being validated against live Supabase data before broader employee rollout.</p></section>
  </div></main>
}
