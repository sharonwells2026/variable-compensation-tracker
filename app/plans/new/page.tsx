...
export default function NewPlanPage(){
 ...
 const submit=async(e:FormEvent)=>{
   ...
   router.push(versionId?`/plans?version=${versionId}`:"/plans");
 };
 return <main className="plan-workspace"><div className="plan-page-shell">
   ...
   <header className="plan-page-header"><div><span className="plan-kicker">PLAN CONFIGURATION</span><h1>Create compensation plan</h1><p>Start with the plan shell. You will add compensation components, Earned rules, eligibility conditions, and assignments next.</p></div></header>
   <div className="plan-lifecycle-strip"><div><span>1</span><b>Plan</b><small>Name, dates, scope</small></div><div><span>2</span><b>Components</b><small>What it pays</small></div><div><span>3</span><b>Earned</b><small>Qualification rules</small></div><div><span>4</span><b>Eligible</b><small>Additional condition</small></div><div><span>5</span><b>Assign & activate</b><small>People and effective dates</small></div></div>
   <form onSubmit={submit} className="plan-editor-card">
     ...
     <div className="plan-form-grid two">
       <label>Plan name<input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Wes Morris 2027 Variable Compensation Plan" required/></label>
       <label>Plan code<input value={code} onChange={e=>setCode(e.target.value.toUpperCase().replace(/[^A-Z0-9_]/g,"_"))} placeholder="WES_2027" required/><small>Stable internal identifier. Use letters, numbers, and underscores.</small></label>
       <label>Effective start date<input type="date" value={startDate} onChange={e=>setStartDate(e.target.value)} required/></label>
     ...
     <div className="plan-info-row"><Check size={16}/><span>The new plan will be created as <b>Draft v1</b>. You can safely build and test it before activation.</span></div>
     <div className="plan-actions"><Link href="/plans" className="plan-button secondary">Cancel</Link><button disabled={saving} className="plan-button primary" type="submit"><Save size={16}/>{saving?"Creating…":"Create draft plan"}</button></div>
   </form>
 </div></main>;
}
