...
export default function ValueEditor({field,operator,value,onChange}:{field?:RuleFieldMeta;operator?:RuleOperatorMeta;value:string;onChange:(value:string)=>void}){
  ...
  const filtered=useMemo(()=>{const q=search.trim().toLowerCase();return q?options.filter(o=>`${o.label} ${o.value}`.toLowerCase().includes(q)):options},[options,search]);
  ...
  if(options.length>0&&arity==="single")return <select value={value} onChange={e=>onChange(e.target.value)} style={inputStyle}><option value="">Choose value…</option>{options.map(o=><option key={o.value} value={o.value}>{o.label}</option>)}</select>;
  ...
  if(options.length>0&&arity==="multiple"){
    ...
    return <div style={{position:"relative"}}>
      <button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open} style={{width:"100%",textAlign:"left",padding:"9px 10px",border:"1px solid #cad5df",borderRadius:8,background:"white",cursor:"pointer",display:"flex",justifyContent:"space-between",gap:8}}><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{selected.length?selectedLabels.join(", "):"Choose values…"}</span><span>{open?"▲":"▼"}</span></button>
      {open&&<div style={{position:"absolute",zIndex:40,top:"calc(100% + 4px)",left:0,right:0,border:"1px solid #cad5df",borderRadius:8,background:"white",boxShadow:"0 8px 24px rgba(5,27,52,.12)",overflow:"hidden"}}>
        <div style={{padding:8,borderBottom:"1px solid #e7edf3"}}><input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder="Search available values…" style={{...inputStyle,padding:"7px 8px",border:"1px solid #d7e1eb",borderRadius:6}}/></div>
        <div style={{maxHeight:220,overflowY:"auto",padding:6}}>{filtered.length===0?<div style={{padding:8,color:"#718096",fontSize:12}}>No matching values.</div>:filtered.map(o=><label key={o.value} style={{display:"flex",gap:8,alignItems:"flex-start",padding:"8px",borderRadius:6,cursor:"pointer"}}><input type="checkbox" checked={selected.includes(o.value)} onChange={e=>toggle(o.value,e.target.checked)}/><span><span style={{display:"block",fontSize:13}}>{o.label}</span>{o.value!==o.label&&<code style={{fontSize:10,color:"#7a8794"}}>{o.value}</code>}</span></label>)}</div>
