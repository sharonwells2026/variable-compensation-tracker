...
export default function ValueEditor({field,operator,value,onChange}:{field?:RuleFieldMeta;operator?:RuleOperatorMeta;value:string;onChange:(value:string)=>void}){
  const arity=operator?.value_arity||"single",options=field?.options||[],type=field?.data_type||"string";
  ...
  const person=isPersonField(field),stage=isStageField(field),wonValues=stage?options.filter(o=>/\bwon\b/i.test(o.label)).map(o=>o.value):[];
  ...
  if(options.length>0&&arity==="multiple"){
    ...
    return <div style={{position:"relative"}}>
      <button type="button" onClick={()=>setOpen(v=>!v)} aria-expanded={open} style={{width:"100%",textAlign:"left",padding:"8px 10px",border:"1px solid #cad5df",borderRadius:8,background:"white",cursor:"pointer",display:"flex",justifyContent:"space-between",gap:8}}><span style={{overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{selected.length?`${selected.length} selected`:"Choose values…"}</span><span aria-hidden>{open?"▲":"▼"}</span></button>
      {selectedOptions.length>0&&<div style={{display:"flex",gap:5,flexWrap:"wrap",marginTop:6}}>{selectedOptions.map(o=><span key={o.value} style={{display:"inline-flex",alignItems:"center",gap:5,padding:"4px 7px",border:"1px solid #d7e1eb",borderRadius:999,background:"#f8fbfe",fontSize:10.5,fontWeight:700}}>{person&&<span style={{width:18,height:18,borderRadius:"50%",display:"grid",placeItems:"center",background:"#dceeff",color:"#1267a5",fontSize:8}}>{initials(o.label)}</span>}{o.label}<button type="button" aria-label={`Remove ${o.label}`} onClick={()=>toggle(o.value,false)} style={{border:0,background:"transparent",cursor:"pointer",padding:0,color:"#667085"}}>×</button></span>)}</div>}
      {open&&<div style={{position:"absolute",zIndex:40,top:"calc(100% + 4px)",left:0,right:0,minWidth:260,maxHeight:"min(420px,65vh)",display:"flex",flexDirection:"column",border:"1px solid #cad5df",borderRadius:8,background:"white",boxShadow:"0 8px 24px rgba(5,27,52,.12)",overflow:"hidden"}}>
        <div style={{padding:7,borderBottom:"1px solid #e7edf3",flex:"0 0 auto"}}><input autoFocus value={search} onChange={e=>setSearch(e.target.value)} placeholder={stage?"Search stages…":"Search available values…"} style={{...inputStyle,padding:"7px 8px",border:"1px solid #d7e1eb",borderRadius:6}}/>{wonValues.length>1&&<button type="button" onClick={()=>setMany(wonValues)} style={{marginTop:6,border:"1px solid #b9c5d1",borderRadius:7,padding:"5px 8px",background:"#f8fbfe",fontSize:11,fontWeight:800,cursor:"pointer"}}>Select any Won stage</button>}</div>
        <div className="plan-compact-listbox" style={{flex:"1 1 auto"}}>{filtered.length===0?<div style={{padding:8,color:"#718096",fontSize:12}}>No matching values.</div>:Object.entries(grouped).map(([group,items])=><div key={group}>{(stage||group!=="Available values")&&<div style={{padding:"6px 8px 2px",fontSize:9.5,fontWeight:800,letterSpacing:.6,color:"#718096",textTransform:"uppercase",textAlign:"left"}}>{group}</div>}{items.map(o=><label key={o.value}><input type="checkbox" checked={selected.includes(o.value)} onChange={e=>toggle(o.value,e.target.checked)}/>{person&&<span style={{width:22,height:22,borderRadius:"50%",display:"grid",placeItems:"center",background:"#eaf5ff",color:"#1267a5",fontSize:9,fontWeight:800}}>{initials(o.label)}</span>}<span style={{lineHeight:1.2,textAlign:"left"}}>{o.label}</span></label>)}</div>)}</div>
        <div className="plan-popover-footer"><span style={{marginRight:"auto",alignSelf:"center",fontSize:11,color:"#647184"}}>{selected.length} selected</span><button type="button" className="plan-button brand" onClick={()=>{setOpen(false);setSearch("")}}>Apply</button></div>
      </div>}
    </div>
  }
