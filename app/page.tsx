...
export default function Home(){
 const router=useRouter();const[message,setMessage]=useState("Opening your compensation workspace…");
 useEffect(()=>{(async()=>{if(!supabase){setMessage("Database connection unavailable.");return}const{data:{session}}=await supabase.auth.getSession();if(!session){setMessage("Please sign in to access the compensation tracker.");return}const{data,error}=await supabase.rpc("get_current_user_access");if(error){setMessage(error.message);return}const access=(data||{}) as Access;const roles=access.roles||[];
 ...
 return <main style={{minHeight:"100vh",display:"grid",placeItems:"center",background:"#f5f7fa",fontFamily:"Inter,Arial,sans-serif",color:"#051b34"}}><div style={{textAlign:"center",padding:28}}><div style={{fontWeight:900,letterSpacing:1.2,color:"#2095f3",fontSize:13}}>ENGAGIFII COMPENSATION</div><h1 style={{margin:"10px 0 6px"}}>Compensation Tracker</h1><p style={{color:"#647184"}}>{message}</p></div></main>
}
