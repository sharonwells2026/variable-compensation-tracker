...
begin
  ...
  select pv.comp_plan_id,pv.status into selected_plan_id,selected_status from public.comp_plan_versions pv where pv.id=selected_plan_version_id for update;
  ...
  if earning_count>0 or assignment_count>0 then raise exception 'This draft has financial or historical dependencies and cannot be deleted. Retire or preserve it instead.' using errcode='42501'; end if;
  ...
  insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,previous_state,new_state) values((select auth.uid()),'comp_plan_draft_deleted','An unused draft compensation plan version was deleted.',jsonb_build_object('plan_version_id',selected_plan_version_id,'plan_id',selected_plan_id),'null'::jsonb);
