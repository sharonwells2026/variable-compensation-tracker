...
begin
  ...
  with assignment as (
    select da.effective_start_date,da.effective_end_date
    from public.comp_plan_draft_assignments da
    where version_status='draft' and da.employee_id=target_employee_id and da.plan_version_id=plan_version
    union all
    select epa.effective_start_date,epa.effective_end_date
    from public.employee_plan_assignments epa
    where version_status<>'draft' and epa.employee_id=target_employee_id and epa.plan_version_id=plan_version
  ), source_components as (
    select sc.id,sc.component_code,sc.measurement_source
    from public.comp_plan_components sc
    where sc.plan_version_id=plan_version and sc.is_active=true and sc.id<>target_component_id and sc.component_code=any(component_codes)
  ), raw as (
    select sc.id source_component_id,d.hubspot_deal_id,
      ...
      case when exists(select 1 from public.comp_user_attribution_rules mr where mr.plan_component_id=sc.id and mr.attribution_purpose='metric' and mr.metric_key='book_of_business' and mr.is_active=true)
        ...
        else private.resolve_comp_user_attribution(target_employee_id,sc.id,d.hubspot_deal_id,'earning',null) end as attribution
    ...
    where extract(year from d.close_date)::int=target_year
      and d.close_date between greatest(a.effective_start_date,pv.effective_start_date,dr.effective_start_date)
      and least(coalesce(a.effective_end_date,'infinity'::date),coalesce(pv.effective_end_date,'infinity'::date),coalesce(dr.effective_end_date,'infinity'::date))
  ), deduped as (
