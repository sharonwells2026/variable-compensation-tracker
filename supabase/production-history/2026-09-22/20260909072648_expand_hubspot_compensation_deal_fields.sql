alter table public.hubspot_deals
  add column if not exists new_business_arr numeric,
  add column if not exists expansion_arr numeric,
  add column if not exists renewal_arr numeric,
  add column if not exists non_recurring_revenue numeric,
  add column if not exists opportunity_type text,
  add column if not exists purchased_modules text,
  add column if not exists solutions_advisor_id text,
  add column if not exists business_development_owner_id text,
  add column if not exists qdc_completed_date date,
  add column if not exists is_closed_won boolean;
...
begin
  ...
  for request_item in
    select pipeline_id,deal_year,make_date(deal_year,1,1) as start_date,make_date(deal_year+1,1,1) as end_date
    from (
    ...
    loop
      request_body := jsonb_build_object(
        'filterGroups',jsonb_build_array(jsonb_build_object('filters',jsonb_build_array(
          jsonb_build_object('propertyName','pipeline','operator','EQ','value',request_item.pipeline_id),
