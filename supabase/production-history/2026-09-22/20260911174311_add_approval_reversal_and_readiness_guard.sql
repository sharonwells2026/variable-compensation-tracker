...
begin
  ...
  where comp_earning_id=target_earning_id and status='approved'::public.approval_status
  order by approval_order desc, completed_at desc nulls last
  limit 1
  for update;
  if not found then raise exception 'No completed approval step could be reopened.' using errcode='P0002'; end if;

  update public.approval_requests
  set status='pending'::public.approval_status, completed_at=null,
      notes=concat_ws(E'\n',nullif(notes,''),'Approval reversed: '||normalized_reason)
  where id=request_row.id;

  delete from public.comp_earning_handoff_tasks
  where comp_earning_id=target_earning_id and status in ('pending','completed');

  update public.comp_earnings
  set approved_amount=0,
      payment_status='not_payable'::public.payment_status,
      expected_payment_date=null,
      expected_pay_period_label=null,
      expected_pay_period_start=null,
      expected_pay_period_end=null,
      finance_accepted_by=null,
      finance_accepted_at=null,
  ...
  values(actor,'comp_earning_approval_reversed',normalized_reason,
    jsonb_build_object('earning_id',target_earning_id,'approval_request_id',request_row.id,'approved_amount',earning_row.approved_amount,'payment_status',earning_row.payment_status),
    jsonb_build_object('earning_id',target_earning_id,'approval_request_id',request_row.id,'approval_status','pending','approved_amount',0,'payment_status','not_payable'),
