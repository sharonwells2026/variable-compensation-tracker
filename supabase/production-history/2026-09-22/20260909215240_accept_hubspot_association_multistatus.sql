    ...
    for target_type in select unnest(array['companies','deals']) loop
      ...
      loop
        ...
        if r.status in (200,207) then
          ...
          if r.status=207 and jsonb_array_length(coalesce(body->'errors','[]'::jsonb))>0 then
            assoc_warnings:=assoc_warnings||jsonb_build_array(jsonb_build_object(
              'target_type',target_type,
              'batch',batch_no,
              'http_status',r.status,
              'partial_errors',body->'errors'
            ));
          end if;
        else
          assoc_warnings:=assoc_warnings||jsonb_build_array(jsonb_build_object(
