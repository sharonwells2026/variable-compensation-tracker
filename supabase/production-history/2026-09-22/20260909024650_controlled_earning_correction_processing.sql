...
declare
  ...
  normalized_reason text := nullif(trim(coalesce(processing_reason,'')),'');
  adj_id uuid;
begin
  ...
  if r.processing_path='direct_correction' then
    ...
    values(actor,'comp_credit_correction_ready_to_apply',normalized_reason,jsonb_build_object('request_id',r.id,'status',r.status),jsonb_build_object('request_id',r.id,'status','ready_to_apply','processing_path',r.processing_path),r.current_employee_id);
    return jsonb_build_object('status','ready_to_apply','processing_path',r.processing_path,'correction_request_id',r.id);
  elsif r.processing_path='return_reopen' then
    ...
    values(e.id,e.employee_id,e.plan_assignment_id,e.plan_component_id,e.source_external_id,'returned_for_attribution_correction',e.payment_status::text,'correction_requested',normalized_reason,'compensation_tracker',jsonb_build_object('manager_approval_status',e.manager_approval_status,'executive_approval_status',e.executive_approval_status,'payment_status',e.payment_status),jsonb_build_object('manager_approval_status','returned','executive_approval_status','returned','payment_status','not_payable','correction_request_id',r.id),true,'pending');
    update public.comp_credit_correction_requests set status='ready_to_apply',resolution_notes=concat_ws(E'\n',resolution_notes,normalized_reason),updated_at=now() where id=r.id;
    return jsonb_build_object('status','ready_to_apply','processing_path',r.processing_path,'correction_request_id',r.id,'earning_id',e.id);
  elsif r.processing_path='paid_adjustment' then
    ...
    insert into public.earning_adjustments(employee_id,comp_earning_id,comp_period_id,adjustment_type,amount,reason,requested_by,approval_status,effective_date,supporting_details)
    values(e.employee_id,e.id,e.comp_period_id,'correction'::public.adjustment_type,-abs(coalesce(e.paid_amount,e.earning_amount,0)),normalized_reason,actor,'pending'::public.approval_status,current_date,jsonb_build_object('correction_request_id',r.id,'original_paid_amount',e.paid_amount,'requested_employee_id',r.requested_employee_id,'requested_credit_percentage',r.requested_credit_percentage,'purpose','preserve_paid_history_and_reverse_for_replacement')) returning id into adj_id;
    update public.comp_credit_correction_requests set status='adjustment_pending',resolution_notes=concat_ws(E'\n',resolution_notes,normalized_reason),updated_at=now() where id=r.id;
    insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,previous_state,new_state,related_employee_id)
    values(actor,'paid_comp_credit_correction_adjustment_created',normalized_reason,jsonb_build_object('request_id',r.id,'earning_id',e.id,'paid_amount',e.paid_amount),jsonb_build_object('request_id',r.id,'adjustment_id',adj_id,'status','adjustment_pending'),e.employee_id);
    return jsonb_build_object('status','adjustment_pending','processing_path',r.processing_path,'correction_request_id',r.id,'adjustment_id',adj_id,'earning_id',e.id);
  end if;
  raise exception 'Unknown correction processing path.' using errcode='22023';
end;
$$;
