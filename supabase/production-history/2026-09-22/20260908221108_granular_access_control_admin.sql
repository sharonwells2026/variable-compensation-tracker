...
begin
 ...
 select jsonb_build_object(
  ...
  'active_users',coalesce((select jsonb_agg(jsonb_build_object('user_id',p.id,'email',p.email,'full_name',coalesce(emp.full_name,p.email),'roles',coalesce((select jsonb_agg(ur.role_key order by ur.role_key) from private.user_roles ur where ur.user_id=p.id),'[]'::jsonb),'overrides',coalesce((select jsonb_agg(jsonb_build_object('permission_key',o.permission_key,'allowed',o.allowed,'reason',o.reason) order by o.permission_key) from private.user_permission_overrides o where o.user_id=p.id),'[]'::jsonb)) order by coalesce(emp.full_name,p.email)) from public.profiles p left join public.employees emp on emp.user_id=p.id where p.is_active=true),'[]'::jsonb),
  'draft_users',coalesce((select jsonb_agg(jsonb_build_object('draft_user_id',d.id,'email',d.email,'full_name',d.full_name,'status',d.status,'roles',coalesce((select jsonb_agg(dr.role_key order by dr.role_key) from private.draft_user_roles dr where dr.draft_user_id=d.id),'[]'::jsonb),'overrides',coalesce((select jsonb_agg(jsonb_build_object('permission_key',o.permission_key,'allowed',o.allowed,'reason',o.reason) order by o.permission_key) from private.draft_user_permission_overrides o where o.draft_user_id=d.id),'[]'::jsonb)) order by d.full_name) from private.draft_users d where d.status not in ('activated','cancelled')),'[]'::jsonb)
 ) into result;
 return result;
end;$function$;
revoke all on function public.get_access_control_admin_data() from public; grant execute on function public.get_access_control_admin_data() to authenticated;
...
revoke all on function public.admin_set_role_permission(text,text,boolean) from public; grant execute on function public.admin_set_role_permission(text,text,boolean) to authenticated;
...
as $function$ declare actor uuid:=(select auth.uid()); normalized text:=lower(trim(coalesce(requested_state,''))); begin
 ...
 insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,new_state) values(actor,'user_permission_override_changed',nullif(trim(coalesce(requested_reason,'')),''),jsonb_build_object('target_user_id',target_user_id,'permission_key',target_permission_key,'state',normalized));
 return jsonb_build_object('status','saved','state',normalized);
end;$function$;
revoke all on function public.admin_set_user_permission_override(uuid,text,text,text) from public; grant execute on function public.admin_set_user_permission_override(uuid,text,text,text) to authenticated;
...
revoke all on function public.admin_set_draft_user_permission_override(uuid,text,text,text) from public; grant execute on function public.admin_set_draft_user_permission_override(uuid,text,text,text) to authenticated;
...
as $function$ declare actor uuid:=(select auth.uid()); admin_count integer; begin
 ...
 insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,new_state) values(actor,'user_role_changed','User role assignment changed.',jsonb_build_object('target_user_id',target_user_id,'role_key',target_role_key,'assigned',requested_assigned));
 return jsonb_build_object('status','saved');
end;$function$;
revoke all on function public.admin_set_user_role(uuid,text,boolean) from public; grant execute on function public.admin_set_user_role(uuid,text,boolean) to authenticated;
...
revoke all on function public.admin_set_draft_user_role(uuid,text,boolean) from public; grant execute on function public.admin_set_draft_user_role(uuid,text,boolean) to authenticated;