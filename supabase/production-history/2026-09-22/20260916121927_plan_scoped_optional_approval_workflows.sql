...
create or replace function private.install_plan_approval_workflows(target_plan_version_id uuid,requested_by_user uuid)
returns integer language plpgsql security definer set search_path=''
as $function$
declare assignment record; step record; workflow_id uuid; workflow_count integer:=0; plan_name text;
begin
  select p.name into plan_name from public.comp_plan_versions pv join public.comp_plans p on p.id=pv.comp_plan_id where pv.id=target_plan_version_id;
  if plan_name is null then raise exception 'Plan version not found.' using errcode='22023'; end if;
  if not exists(select 1 from public.comp_plan_approval_steps s where s.plan_version_id=target_plan_version_id and s.is_required=true) then raise exception 'At least one required plan approval step is needed before activation.' using errcode='23514'; end if;
  for assignment in select da.employee_id,da.effective_start_date,da.effective_end_date from public.comp_plan_draft_assignments da where da.plan_version_id=target_plan_version_id order by da.employee_id loop
    delete from public.employee_approval_workflow_versions w where w.employee_id=assignment.employee_id and w.plan_version_id=target_plan_version_id and w.status='draft';
    select w.id into workflow_id from public.employee_approval_workflow_versions w where w.employee_id=assignment.employee_id and w.plan_version_id=target_plan_version_id and w.status='active' order by w.created_at desc limit 1;
    if workflow_id is null then
      insert into public.employee_approval_workflow_versions(employee_id,workflow_name,effective_start_date,effective_end_date,status,notes,created_by,plan_version_id)
      values(assignment.employee_id,plan_name||' approval workflow',assignment.effective_start_date,assignment.effective_end_date,'active','Generated from the activated compensation plan approval workflow.',requested_by_user,target_plan_version_id) returning id into workflow_id;
      for step in select s.* from public.comp_plan_approval_steps s where s.plan_version_id=target_plan_version_id order by s.approval_order loop
        insert into public.employee_approval_chains(employee_id,workflow_version_id,approval_order,step_name,approver_employee_id,approver_user_id,backup_approver_employee_id,backup_approver_user_id,approval_level,is_required,effective_start_date,effective_end_date,conditions,created_by)
        values(assignment.employee_id,workflow_id,step.approval_order,step.step_name,step.approver_employee_id,private.resolve_employee_approver_user(step.approver_employee_id),step.backup_approver_employee_id,private.resolve_employee_approver_user(step.backup_approver_employee_id),step.approval_level,step.is_required,assignment.effective_start_date,assignment.effective_end_date,coalesce(step.conditions,'{}'::jsonb)||jsonb_build_object('plan_version_id',target_plan_version_id),requested_by_user);
      end loop;
...
as $function$
declare
  earning_row public.comp_earnings%rowtype; earning_plan_version_id uuid; previous_required_order integer:=0; next_required public.employee_approval_chains%rowtype; optional_chain public.employee_approval_chains%rowtype; new_request_id uuid; optional_request_id uuid; has_plan_workflow boolean:=false;
begin
  ...
  select c.plan_version_id into earning_plan_version_id from public.comp_plan_components c where c.id=earning_row.plan_component_id;
  select exists(select 1 from public.employee_approval_workflow_versions w where w.employee_id=earning_row.employee_id and w.plan_version_id=earning_plan_version_id and w.status in ('active','ended') and earning_row.earned_date>=w.effective_start_date and (w.effective_end_date is null or earning_row.earned_date<=w.effective_end_date)) into has_plan_workflow;
  if exists(select 1 from public.approval_requests r where r.comp_earning_id=target_earning_id and r.status='pending'::public.approval_status and coalesce((r.chain_snapshot->>'is_required')::boolean,true)) then raise exception 'This earning already has a pending required approval request.' using errcode='23505'; end if;
  select coalesce(max(r.approval_order),0) into previous_required_order from public.approval_requests r where r.comp_earning_id=target_earning_id and coalesce((r.chain_snapshot->>'is_required')::boolean,true) and r.status='approved'::public.approval_status;
  select c.* into next_required from public.employee_approval_chains c join public.employee_approval_workflow_versions w on w.id=c.workflow_version_id where c.employee_id=earning_row.employee_id and c.is_required=true and c.approval_order>previous_required_order and w.status in ('active','ended') and earning_row.earned_date>=c.effective_start_date and (c.effective_end_date is null or earning_row.earned_date<=c.effective_end_date) and ((has_plan_workflow and w.plan_version_id=earning_plan_version_id) or (not has_plan_workflow and w.plan_version_id is null)) order by c.approval_order limit 1;
  for optional_chain in select c.* from public.employee_approval_chains c join public.employee_approval_workflow_versions w on w.id=c.workflow_version_id where c.employee_id=earning_row.employee_id and c.is_required=false and coalesce(c.conditions->>'notify_when','turn')='immediate' and w.status in ('active','ended') and earning_row.earned_date>=c.effective_start_date and (c.effective_end_date is null or earning_row.earned_date<=c.effective_end_date) and ((has_plan_workflow and w.plan_version_id=earning_plan_version_id) or (not has_plan_workflow and w.plan_version_id is null)) and not exists(select 1 from public.approval_requests r where r.comp_earning_id=target_earning_id and r.approval_order=c.approval_order) order by c.approval_order loop
    ...
    values(target_earning_id,optional_chain.approval_level,optional_chain.approval_order,optional_chain.approver_user_id,optional_chain.backup_approver_user_id,requested_by_user,now(),'pending'::public.approval_status,jsonb_build_object('employee_approval_chain_id',optional_chain.id,'employee_id',optional_chain.employee_id,'approval_order',optional_chain.approval_order,'approval_level',optional_chain.approval_level,'is_required',false,'approver_employee_id',optional_chain.approver_employee_id,'approver_user_id',optional_chain.approver_user_id,'backup_approver_employee_id',optional_chain.backup_approver_employee_id,'backup_approver_user_id',optional_chain.backup_approver_user_id,'effective_start_date',optional_chain.effective_start_date,'effective_end_date',optional_chain.effective_end_date,'earning_earned_date',earning_row.earned_date,'snapshotted_at',now(),'assignee_activation_required',optional_chain.approver_user_id is null,'notify_when','immediate')) returning id into optional_request_id;
    if optional_chain.approver_user_id is not null then insert into public.app_notifications(recipient_user_id,notification_type,title,message,priority,related_record_type,related_record_id,action_url,email_status,email_queued_at) values(optional_chain.approver_user_id,'compensation_optional_review','Optional compensation review','A compensation earning is available for your optional review. Your response does not block the required approval workflow.','normal','approval_request',optional_request_id::text,'/submissions','queued',now()); end if;
  end loop;
  for optional_chain in select c.* from public.employee_approval_chains c join public.employee_approval_workflow_versions w on w.id=c.workflow_version_id where c.employee_id=earning_row.employee_id and c.is_required=false and coalesce(c.conditions->>'notify_when','turn')='turn' and c.approval_order>previous_required_order and (next_required.id is null or c.approval_order<next_required.approval_order) and w.status in ('active','ended') and earning_row.earned_date>=c.effective_start_date and (c.effective_end_date is null or earning_row.earned_date<=c.effective_end_date) and ((has_plan_workflow and w.plan_version_id=earning_plan_version_id) or (not has_plan_workflow and w.plan_version_id is null)) and not exists(select 1 from public.approval_requests r where r.comp_earning_id=target_earning_id and r.approval_order=c.approval_order) order by c.approval_order loop
    insert into public.approval_requests(comp_earning_id,approval_level,approval_order,requested_from,backup_requested_from,requested_by,requested_at,status,chain_snapshot)
    values(target_earning_id,optional_chain.approval_level,optional_chain.approval_order,optional_chain.approver_user_id,optional_chain.backup_approver_user_id,requested_by_user,now(),'pending'::public.approval_status,jsonb_build_object('employee_approval_chain_id',optional_chain.id,'employee_id',optional_chain.employee_id,'approval_order',optional_chain.approval_order,'approval_level',optional_chain.approval_level,'is_required',false,'approver_employee_id',optional_chain.approver_employee_id,'approver_user_id',optional_chain.approver_user_id,'backup_approver_employee_id',optional_chain.backup_approver_employee_id,'backup_approver_user_id',optional_chain.backup_approver_user_id,'effective_start_date',optional_chain.effective_start_date,'effective_end_date',optional_chain.effective_end_date,'earning_earned_date',earning_row.earned_date,'snapshotted_at',now(),'assignee_activation_required',optional_chain.approver_user_id is null,'notify_when','turn')) returning id into optional_request_id;
    if optional_chain.approver_user_id is not null then insert into public.app_notifications(recipient_user_id,notification_type,title,message,priority,related_record_type,related_record_id,action_url,email_status,email_queued_at) values(optional_chain.approver_user_id,'compensation_optional_review','Optional compensation review','A compensation earning is available for your optional review. Your response does not block the required approval workflow.','normal','approval_request',optional_request_id::text,'/submissions','queued',now()); end if;
  end loop;
  if next_required.id is null then return null; end if;
  insert into public.approval_requests(comp_earning_id,approval_level,approval_order,requested_from,backup_requested_from,requested_by,requested_at,status,chain_snapshot)
  values(target_earning_id,next_required.approval_level,next_required.approval_order,next_required.approver_user_id,next_required.backup_approver_user_id,requested_by_user,now(),'pending'::public.approval_status,jsonb_build_object('employee_approval_chain_id',next_required.id,'employee_id',next_required.employee_id,'approval_order',next_required.approval_order,'approval_level',next_required.approval_level,'is_required',true,'approver_employee_id',next_required.approver_employee_id,'approver_user_id',next_required.approver_user_id,'backup_approver_employee_id',next_required.backup_approver_employee_id,'backup_approver_user_id',next_required.backup_approver_user_id,'effective_start_date',next_required.effective_start_date,'effective_end_date',next_required.effective_end_date,'earning_earned_date',earning_row.earned_date,'snapshotted_at',now(),'assignee_activation_required',next_required.approver_user_id is null)) returning id into new_request_id;
  if next_required.approver_user_id is not null then insert into public.app_notifications(recipient_user_id,notification_type,title,message,priority,related_record_type,related_record_id,action_url,email_status,email_queued_at) values(next_required.approver_user_id,'compensation_approval_required','Compensation approval required','A compensation earning is waiting for your approval.','high','approval_request',new_request_id::text,'/submissions','queued',now()); end if;
  if next_required.backup_approver_user_id is not null then insert into public.app_notifications(recipient_user_id,notification_type,title,message,priority,related_record_type,related_record_id,action_url,email_status,email_queued_at) values(next_required.backup_approver_user_id,'compensation_approval_backup','Backup compensation approval','You are the backup approver for a compensation earning.','normal','approval_request',new_request_id::text,'/submissions','queued',now()); end if;
...
as $function$
  select exists(select 1 from public.approval_requests r where r.comp_earning_id=target_earning_id and coalesce((r.chain_snapshot->>'is_required')::boolean,true)) and not exists(select 1 from public.approval_requests r where r.comp_earning_id=target_earning_id and coalesce((r.chain_snapshot->>'is_required')::boolean,true) and r.status<>'approved'::public.approval_status);
$function$;
...
as $function$
declare actor uuid:=auth.uid(); request_row public.approval_requests%rowtype; earning_row public.comp_earnings%rowtype; next_request_id uuid; final_approval boolean:=false; handoff_count int:=0; normalized_comments text:=nullif(trim(coalesce(action_comments,'')),''); is_admin boolean:=false; request_is_required boolean:=true;
begin
 ...
 if not request_is_required then
   insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,new_state,related_employee_id) values(actor,'comp_earning_optional_review_recorded',normalized_comments,jsonb_build_object('earning_id',earning_row.id,'approval_request_id',target_approval_request_id,'action',requested_action,'approval_order',request_row.approval_order,'acted_as_system_admin',is_admin),earning_row.employee_id);
...
returns jsonb language plpgsql security definer set search_path=''
as $function$
declare selected_plan_id uuid; selected_start date; selected_end date; conflicting_active_id uuid; conflicting_active_start date; readiness jsonb; promoted_count integer:=0; installed_workflow_count integer:=0;
begin
  ...
  insert into public.app_access_audit_events(actor_user_id,event_type,event_reason,new_state) values(auth.uid(),'comp_plan_version_activated','A readiness-validated compensation plan version was activated; draft applicability and its plan-scoped approval workflow were promoted together.',jsonb_build_object('plan_id',selected_plan_id,'plan_version_id',selected_plan_version_id,'effective_start_date',selected_start,'previous_active_version_id',conflicting_active_id,'promoted_assignment_count',promoted_count,'installed_approval_workflow_count',installed_workflow_count,'readiness',readiness));
