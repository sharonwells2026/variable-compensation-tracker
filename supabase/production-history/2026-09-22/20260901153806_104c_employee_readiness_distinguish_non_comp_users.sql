...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'compensation_participant',flags.compensation_participant,
      'plan_name',cp.name,
      'plan_status',pv.status::text,
      'plan_effective_start_date',pa.effective_start_date,
      'plan_effective_end_date',pa.effective_end_date,
      'earnings_eligibility_date',pa.earnings_eligibility_date,
      'workflow_name',wv.workflow_name,
      'workflow_status',wv.status,
      'readiness',case
        when (pr.is_active or d.id is not null)
         and (not flags.compensation_participant or (pa.id is not null and pa.earnings_eligibility_date is not null and wv.id is not null))
        then case when pr.is_active then 'ready' else 'preinvite_ready' end
        else 'needs_setup'
      end,
    ...
    left join lateral (
      select (
        exists(select 1 from public.employee_plan_assignments a0 where a0.employee_id=e.id)
        or exists(select 1 from private.draft_user_roles dr where dr.draft_user_id=d.id and dr.role_key='employee')
        or exists(select 1 from private.user_roles ur where ur.user_id=pr.id and ur.role_key='employee')
      ) as compensation_participant
    ) flags on true
...
comment on function public.get_employee_administration_data() is
'104C employee administration readiness model. Non-compensation operational users do not require a compensation plan or personal approval workflow; pre-invite drafts are surfaced separately from active app access.';