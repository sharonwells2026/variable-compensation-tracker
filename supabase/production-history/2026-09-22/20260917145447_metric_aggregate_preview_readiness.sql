  ...
  loop
    if c.measurement_source in ('amount','average_arr','first_year_arr','one_time_fee','total_contract_value') then
      ...
      if metric_rule_id is not null then
        if not exists(select 1 from public.comp_rule_sets rs where rs.id=metric_rule_id and rs.plan_component_id=c.id and rs.purpose='metric_qualification') then
        ...
        elsif not exists(select 1 from public.comp_rule_expression_nodes n where n.rule_set_id=metric_rule_id and n.parent_node_id is null) then
