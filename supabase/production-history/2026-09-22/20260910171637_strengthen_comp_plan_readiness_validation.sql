...
begin
  ...
  select pv.status::text, pv.effective_start_date into v_status, v_start
  from public.comp_plan_versions pv where pv.id=selected_plan_version_id;
  if v_status is null then raise exception 'Plan version not found.' using errcode='22023'; end if;

  select count(*) into v_component_count from public.comp_plan_components c
  where c.plan_version_id=selected_plan_version_id and c.is_active=true;
  if v_component_count=0 then
    v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','no_components','section','earning_types','message','Add at least one active Earning Type.'));
  end if;

  select count(*) into v_assignment_count from public.comp_plan_draft_assignments da
  where da.plan_version_id=selected_plan_version_id;
  if v_status='draft' and v_assignment_count=0 then
    v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','no_applicability','section','applicability','message','Choose at least one employee this plan will apply to.'));
  end if;
  ...
  for c in
    select id,name,component_code,calculation_type,measurement_source,measurement_period,maximum_payout,rule_configuration,additional_eligibility_waiting_days
  ...
  loop
    if nullif(trim(coalesce(c.calculation_type,'')),'') is null then
      v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_calculation_type','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs a payout method.',c.name)));
    end if;
    if nullif(trim(coalesce(c.measurement_source,'')),'') is null then
      v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_measurement_source','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs a payment basis.',c.name)));
    end if;
    if nullif(trim(coalesce(c.measurement_period,'')),'') is null then
      v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_measurement_period','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs a calculation frequency.',c.name)));
    end if;
    if c.calculation_type in ('percentage','tiered_percentage') then
      if c.calculation_type='percentage' and (c.rule_configuration->>'rate') is null then
        v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_rate','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs a percentage rate.',c.name)));
      elsif c.calculation_type='percentage' and coalesce((c.rule_configuration->>'rate')::numeric,-1) < 0 then
        v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','invalid_rate','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s has an invalid percentage rate.',c.name)));
      elsif c.calculation_type='tiered_percentage' and jsonb_array_length(coalesce(c.rule_configuration->'tiers','[]'::jsonb))=0 then
        v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_tiers','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs at least one payout tier.',c.name)));
      end if;
    elsif c.calculation_type in ('fixed_amount','fixed_amount_per_unit','milestone_bonus','threshold_bonus') then
      if (c.rule_configuration->>'amount') is null and c.calculation_type in ('fixed_amount','fixed_amount_per_unit','milestone_bonus') then
        v_blockers:=v_blockers||jsonb_build_array(jsonb_build_object('code','missing_amount','section','earning_types','component_id',c.id,'component_name',c.name,'message',format('%s needs a payout amount.',c.name)));
      end if;
    end if;
    ...
    if q.id is null then
      ...
      from public.comp_rule_set_preview_runs pr where pr.rule_set_id=q.id order by pr.previewed_at desc limit 1;
    ...
    if nullif(e->>'mode','') is null then
      ...
      if nullif(e->>'rule_set_id','') is null then
        ...
        q:=null;
        select rs.id,rs.updated_at,rs.is_active into q
        from public.comp_rule_sets rs where rs.id=(e->>'rule_set_id')::uuid and rs.plan_component_id=c.id and rs.purpose='eligibility' limit 1;
        if q.id is null then
