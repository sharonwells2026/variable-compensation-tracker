...
begin
  ...
  if selected_calculation_type = 'percentage' then
    if jsonb_typeof(config->'rate_history') = 'array' then
      select rate_item into matched_rate_history from jsonb_array_elements(config->'rate_history') rate_item
      ...
      order by coalesce(nullif(rate_item->>'effective_start_date','')::date,'-infinity'::date) desc limit 1;
      if matched_rate_history is not null then resolved_rate := nullif(matched_rate_history->>'rate','')::numeric; end if;
    else resolved_rate := nullif(config->>'rate','')::numeric; end if;
    ...
    if selected_source_amount is null then calculation_status:='missing_input'; calculation_explanation:='Aggregate attainment value is required.';
      ...
      calculated_amount:=round(milestone_total,2); calculation_explanation:='Milestone attainment at '||selected_source_amount::text||' produced '||milestone_total::text;
    end if;
  else calculation_status:='unsupported'; calculation_explanation:='Calculation type '||selected_calculation_type||' is not supported by the evaluator.'; end if;
  return jsonb_strip_nulls(jsonb_build_object('status',calculation_status,'calculation_type',selected_calculation_type,'calculated_amount',calculated_amount,'source_amount',selected_source_amount,'unit_count',selected_unit_count,'contract_term_years',selected_contract_term_years,'resolved_rate',resolved_rate,'resolved_fixed_amount',resolved_fixed_amount,'matched_tier',matched_tier,'matched_rate_history',matched_rate_history,'threshold_amount',threshold_amount,'threshold_met',threshold_met,'measurement_date',selected_measurement_date,'explanation',calculation_explanation));
end;
$function$;
...
as $function$
declare
  access jsonb; component_row record; aggregate_config jsonb; source_codes jsonb; period_key text; period_start date; period_end date; baseline_amount numeric; benchmark_amount numeric; rows_json jsonb;
begin
  ...
  select c.id,c.plan_version_id,c.name,c.component_code,c.calculation_type,c.measurement_period,c.rule_configuration,pv.status::text as version_status,pv.effective_start_date,pv.effective_end_date into component_row from public.comp_plan_components c join public.comp_plan_versions pv on pv.id=c.plan_version_id where c.id=target_component_id;
  if component_row.id is null then raise exception 'Earning Type not found' using errcode='P0002'; end if;
  aggregate_config:=coalesce(component_row.rule_configuration->'aggregate','{}'::jsonb); source_codes:=coalesce(aggregate_config->'source_component_codes','[]'::jsonb); period_key:=coalesce(nullif(aggregate_config->>'period',''),nullif(component_row.measurement_period,''),'annual'); baseline_amount:=coalesce(nullif(aggregate_config->>'baseline_amount','')::numeric,0); benchmark_amount:=coalesce(nullif(aggregate_config->>'benchmark_amount','')::numeric,nullif(component_row.rule_configuration->>'threshold_amount','')::numeric);
  ...
  with assigned as (
    select da.employee_id,e.full_name from public.comp_plan_draft_assignments da join public.employees e on e.id=da.employee_id where component_row.version_status='draft' and da.plan_version_id=component_row.plan_version_id
    union all select a.employee_id,e.full_name from public.employee_plan_assignments a join public.employees e on e.id=a.employee_id where component_row.version_status<>'draft' and a.plan_version_id=component_row.plan_version_id and selected_measurement_date between a.effective_start_date and coalesce(a.effective_end_date,'infinity'::date)
  ), source_components as (
    select c.id,c.component_code,c.name,c.measurement_source from public.comp_plan_components c where c.plan_version_id=component_row.plan_version_id and c.is_active and c.id<>target_component_id and (jsonb_array_length(source_codes)=0 or source_codes ? c.component_code)
  ), raw_matches as (
    select a.employee_id,a.full_name,sc.id as source_component_id,sc.component_code,sc.name as source_component_name,d.hubspot_deal_id,d.deal_name,d.close_date,case sc.measurement_source when 'average_arr' then d.average_arr when 'first_year_arr' then d.first_year_arr when 'one_time_fee' then d.one_time_fee when 'total_contract_value' then d.total_contract_value else d.amount end as source_amount,private.resolve_comp_user_attribution(a.employee_id,sc.id,d.hubspot_deal_id,'earning',null) as attribution
    ...
    where d.close_date between greatest(period_start,component_row.effective_start_date,coalesce(dr.effective_start_date,period_start)) and least(period_end,coalesce(component_row.effective_end_date,'infinity'::date),coalesce(dr.effective_end_date,'infinity'::date))
  ), deduped as (
    select distinct on(employee_id,source_component_id,hubspot_deal_id) employee_id,full_name,source_component_id,component_code,source_component_name,hubspot_deal_id,deal_name,close_date,source_amount from raw_matches where coalesce((attribution->>'matched')::boolean,false)=true order by employee_id,source_component_id,hubspot_deal_id,close_date desc
  ), totals as (
    select a.employee_id,a.full_name,coalesce(sum(d.source_amount),0) as qualifying_total,count(d.hubspot_deal_id) as qualifying_record_count from assigned a left join deduped d on d.employee_id=a.employee_id group by a.employee_id,a.full_name
  )
  select coalesce(jsonb_agg(jsonb_build_object('employee_id',t.employee_id,'employee_name',t.full_name,'period_start',period_start,'period_end',period_end,'qualifying_total',t.qualifying_total,'baseline_amount',baseline_amount,'attainment_value',t.qualifying_total+baseline_amount,'benchmark_amount',benchmark_amount,'attainment_percent',case when benchmark_amount is null or benchmark_amount=0 then null else round(((t.qualifying_total+baseline_amount)/benchmark_amount)*100,2) end,'qualifying_record_count',t.qualifying_record_count,'calculation',public.evaluate_compensation_rule(component_row.calculation_type,component_row.rule_configuration,t.qualifying_total+baseline_amount,null,null,selected_measurement_date)) order by t.full_name),'[]'::jsonb) into rows_json from totals t;
  return jsonb_build_object('component_id',component_row.id,'component_name',component_row.name,'component_code',component_row.component_code,'plan_version_id',component_row.plan_version_id,'plan_status',component_row.version_status,'period',period_key,'period_start',period_start,'period_end',period_end,'measurement_date',selected_measurement_date,'source_component_codes',source_codes,'baseline_amount',baseline_amount,'benchmark_amount',benchmark_amount,'rows',rows_json);
end;
$function$;
