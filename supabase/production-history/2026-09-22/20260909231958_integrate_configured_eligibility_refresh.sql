...
begin
  ...
  if not (
    ...
    or private.has_permission('earnings.edit')
    or private.has_permission('hubspot.refresh')
  ) then
...
begin
  ...
  if hubspot_result->>'status' in ('completed','skipped') then
    ...
    eligibility_result := jsonb_build_object('status','not_run','reason','Compensation earning refresh did not run.');
  end if;
  ...
  insert into public.app_access_audit_events (
    ...
    'An authorized management user refreshed HubSpot, compensation earnings, and configured eligibility conditions.',
    jsonb_build_object('hubspot', hubspot_result, 'earnings', earnings_result, 'eligibility', eligibility_result)
  );
