...
create table if not exists public.employee_post_approval_steps (
 ...
 workflow_version_id uuid not null references public.employee_approval_workflow_versions(id) on delete cascade,
 employee_id uuid not null references public.employees(id) on delete cascade,
 stage_order integer not null default 1 check(stage_order>0),
 step_name text not null check(nullif(trim(step_name),'') is not null),
 step_type text not null check(step_type in ('finance_acknowledgement','optional_executive_review')),
 assignee_employee_id uuid not null references public.employees(id),
 assignee_user_id uuid references auth.users(id),
 is_required_for_payment boolean not null default false,
 requires_payment_details boolean not null default false,
...
create table if not exists public.comp_earning_handoff_tasks (
 id uuid primary key default gen_random_uuid(),
 comp_earning_id uuid not null references public.comp_earnings(id) on delete cascade,
 workflow_version_id uuid not null references public.employee_approval_workflow_versions(id),
 configured_step_id uuid not null references public.employee_post_approval_steps(id),
...
as $function$
 select case when selected_step_type='finance_acknowledgement'
   then private.draft_employee_has_permission(selected_employee_id,'payments.view')
   else private.draft_employee_has_permission(selected_employee_id,'earnings.approve') end;
$function$;
...
begin
 ...
 for s in select * from public.employee_post_approval_steps where workflow_version_id=w.id and e.earned_date>=effective_start_date and (effective_end_date is null or e.earned_date<=effective_end_date) order by stage_order,id loop
   ...
   if uid is null then
     if s.is_required_for_payment then raise exception 'A required post-approval assignee is not an active application user.' using errcode='22023'; else continue; end if;
   end if;
   ...
   on conflict(comp_earning_id,configured_step_id) do update set assigned_user_id=excluded.assigned_user_id,updated_at=now()
...
begin
 ...
 if approvals_complete and handoffs_complete and e.eligibility_status in ('eligible'::public.eligibility_status,'waived'::public.eligibility_status) and e.eligible_amount>0 and e.approved_amount>0 then
...
begin
 ...
 select * into t from public.comp_earning_handoff_tasks where id=target_task_id for update;
 if not found then raise exception 'Handoff task not found.' using errcode='P0002'; end if;
...
begin
 ...
 if lower(request_row.approval_level)='manager' then update public.comp_earnings set manager_approval_status='approved'::public.approval_status,manager_approved_by=actor,manager_approved_at=now(),manager_approval_notes=normalized_comments where id=earning_row.id;
