...
begin
  ...
  loop
    if coalesce(setting.record_filter_mode,'all')='filtered' then
      ...
      r:=extensions.http(('POST','https://api.hubapi.com/crm/v3/objects/'||target_hubspot_object_type||'/search',array[extensions.http_header('Authorization','Bearer '||hubspot_key),extensions.http_header('Content-Type','application/json')],'application/json',request_body::text)::extensions.http_request);
    else
    ...
    if r.status<>200 then
      return jsonb_build_object('hubspot_object_type',target_hubspot_object_type,'status','warning','http_status',r.status,'reason',left(r.content,300),'records_received',count_records,'filter_mode',coalesce(setting.record_filter_mode,'all'));
    end if;
    ...
    for rec in select value from jsonb_array_elements(coalesce(body->'results','[]'::jsonb)) loop
      ...
      insert into public.hubspot_generic_records(hubspot_object_type,internal_object_key,hubspot_record_id,properties,raw_hubspot_data,hubspot_created_at,hubspot_updated_at,last_synced_at,source_fingerprint,is_archived,in_sync_scope,scope_evaluated_at)
      values(target_hubspot_object_type,obj.internal_object_key,rec->>'id',coalesce(rec->'properties','{}'::jsonb),rec,nullif(rec->>'createdAt','')::timestamptz,nullif(rec->>'updatedAt','')::timestamptz,now(),md5(rec::text),coalesce((rec->>'archived')::boolean,false),true,now())
      on conflict(hubspot_object_type,hubspot_record_id) do update set
        ...
        source_fingerprint=excluded.source_fingerprint,is_archived=excluded.is_archived,in_sync_scope=true,scope_evaluated_at=now();
  ...
  if target_hubspot_object_type='MEETING_EVENT' and count_records>0 then
    ...
    for target_type in select unnest(array['companies','deals']) loop
      ...
      loop
        ...
        from (select id from _synced_generic_ids order by id limit 100 offset batch_no*100) q;
        exit when batch_ids is null;
        r:=extensions.http(('POST','https://api.hubapi.com/crm/v4/associations/'||target_hubspot_object_type||'/'||target_type||'/batch/read',array[extensions.http_header('Authorization','Bearer '||hubspot_key),extensions.http_header('Content-Type','application/json')],'application/json',jsonb_build_object('inputs',batch_ids)::text)::extensions.http_request);
        if r.status=200 then
          body:=r.content::jsonb;
          for assoc_result in select value from jsonb_array_elements(coalesce(body->'results','[]'::jsonb)) loop
            for assoc_to in select value from jsonb_array_elements(coalesce(assoc_result->'to','[]'::jsonb)) loop
              ...
              values(target_hubspot_object_type,obj.internal_object_key,assoc_result->'from'->>'id',case target_type when 'companies' then 'company' else 'deal' end,assoc_to->>'toObjectId',coalesce(assoc_to->'associationTypes'->0->>'typeId','hubspot_defined'),now())
  ...
  return jsonb_build_object('hubspot_object_type',target_hubspot_object_type,'status','completed','records_received',count_records,'created',created_count,'updated',updated_count,'properties_requested',coalesce(array_length(string_to_array(props,','),1),0),'associations_synced',assoc_count,'filter_mode',coalesce(setting.record_filter_mode,'all'),'out_of_scope_records',(select count(*) from public.hubspot_generic_records where hubspot_object_type=target_hubspot_object_type and not in_sync_scope));
end;$function$;
