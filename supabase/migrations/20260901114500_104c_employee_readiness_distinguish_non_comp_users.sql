...
begin
  ...
  from (
    select jsonb_build_object(
      ...
      'plan_name',cp.name,'plan_status',pv.status::text,'plan_effective_start_date',pa.effective_start_date,'plan_effective_end_date',pa.effective_end_date,'earnings_eligibility_date',pa.earnings_eligibility_date,
...
comment on function public.get_employee_administration_data() is '104C employee administration readiness model. Non-compensation operational users do not require a compensation plan or personal approval workflow; pre-invite drafts are surfaced separately from active app access.';
