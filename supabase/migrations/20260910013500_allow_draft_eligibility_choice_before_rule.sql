create or replace function public.save_compensation_plan_component_eligibility(selected_component_id uuid, selected_mode text, selected_waiting_days integer default null, selected_rule_set_id uuid default null, selected_description text default null)
returns jsonb
language plpgsql
security definer
set search_path = ''
...
begin
  ...
  if normalized_mode = 'waiting_period' then
    ...
    select rs.purpose, rs.plan_component_id into rule_purpose, rule_component_id
    from public.comp_rule_sets rs where rs.id = selected_rule_set_id;
  ...
  new_eligibility := jsonb_strip_nulls(jsonb_build_object(
    ...
    'configuration_complete', case when stored_mode='rule' then selected_rule_set_id is not null else true end,
  ...
  set additional_eligibility_waiting_days = case when normalized_mode='waiting_period' then selected_waiting_days else null end,
      rule_configuration = jsonb_set(coalesce(rule_configuration,'{}'::jsonb),'{eligibility}',new_eligibility,true),
  ...
  values ((select auth.uid()),'comp_plan_component_eligibility_updated','The Earned-to-Eligible condition for a draft compensation plan component was updated.',
...
revoke all on function public.save_compensation_plan_component_eligibility(uuid,text,integer,uuid,text) from public, anon;
grant execute on function public.save_compensation_plan_component_eligibility(uuid,text,integer,uuid,text) to authenticated;
