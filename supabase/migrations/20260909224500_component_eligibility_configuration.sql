create or replace function public.save_compensation_plan_component_eligibility(
  selected_component_id uuid,
  selected_mode text,
  selected_waiting_days integer default null,
  selected_rule_set_id uuid default null,
...
begin
  ...
  if normalized_mode not in ('immediate','waiting_period','rule') then
    raise exception 'Eligibility mode must be immediate, waiting_period, or rule.' using errcode = '22023';
  end if;
  ...
  elsif normalized_mode = 'rule' then
  ...
  new_eligibility := jsonb_strip_nulls(jsonb_build_object(
    ...
    'rule_set_id', case when normalized_mode = 'rule' then selected_rule_set_id else null end,
  ...
  insert into public.app_access_audit_events (
    ...
    jsonb_build_object(
      ...
      'eligibility', coalesce(old_configuration->'eligibility','null'::jsonb),
      'additional_eligibility_waiting_days', previous_waiting_days
    ),
    jsonb_build_object(
      'component_id', selected_component_id,
