...
begin
  ...
  where not (
    (
      ...
      and exists(
        select 1
        from public.comp_plan_components pc
        where pc.id=nullif(b->>'component_id','')::uuid
          and (
            ...
            or exists(
              ...
              where dr.plan_component_id=pc.id
  ...
  if approval_step_count=0 then
    blockers:=blockers||jsonb_build_array(jsonb_build_object(
      'code','missing_approval_steps','section','approvals',
      'message','Configure at least one required approval step for this plan.'
    ));
  end if;
    ...
    if c.measurement_source in ('amount','average_arr','first_year_arr','one_time_fee','total_contract_value') then
      if not exists(
        ...
        warnings:=warnings||jsonb_build_array(jsonb_build_object(
          'code','legacy_earned_rules','section','earned','component_id',c.id,'component_name',c.name,
          'message',format('%s is using valid legacy Earned rules. You can keep them for this plan or open Earned conditions to migrate them to the advanced rule model.',c.name)
        ));
      end if;
      ...
      if coalesce(c.rule_configuration->'eligibility','{}'::jsonb)='{}'::jsonb
         and (
           coalesce((c.rule_configuration->>'requires_invoice_paid_date')::boolean,false)=true
           or c.rule_configuration->>'eligibility_condition' in ('paid_stage_and_invoice_paid_date','invoice_paid_date','customer_payment_received')
         ) then
        warnings:=warnings||jsonb_build_array(jsonb_build_object(
          'code','legacy_payment_eligibility','section','eligible','component_id',c.id,'component_name',c.name,
          'message',format('%s is using the existing customer-payment eligibility rule. Open payment conditions if you want to migrate it to the configurable business-condition model.',c.name)
        ));
      end if;
      ...
      if jsonb_array_length(source_codes)=0 then
        blockers:=blockers||jsonb_build_array(jsonb_build_object(
          'code','missing_aggregate_sources','section','earning_types','component_id',c.id,'component_name',c.name,
          'message',format('%s needs the Earning Types that count toward its running total.',c.name)
        ));
      end if;
      if coalesce(nullif(agg->>'benchmark_amount',''),nullif(c.rule_configuration->>'threshold_amount','')) is null then
        blockers:=blockers||jsonb_build_array(jsonb_build_object(
          'code','missing_quota','section','earning_types','component_id',c.id,'component_name',c.name,
          'message',format('%s needs a quota or benchmark threshold.',c.name)
        ));
      end if;
      if coalesce(nullif(agg->>'payout_amount',''),nullif(c.rule_configuration->>'bonus_amount','')) is null then
        blockers:=blockers||jsonb_build_array(jsonb_build_object(
          'code','missing_threshold_bonus','section','earning_types','component_id',c.id,'component_name',c.name,
          'message',format('%s needs the bonus paid when the benchmark is exceeded.',c.name)
        ));
      end if;
      warnings:=warnings||jsonb_build_array(jsonb_build_object(
        'code','aggregate_year_end','section','earning_types','component_id',c.id,'component_name',c.name,
        'message',format('%s tracks attainment during the year and creates the bonus earning at year end if the benchmark is exceeded.',c.name)
      ));
    end if;
  end loop;
