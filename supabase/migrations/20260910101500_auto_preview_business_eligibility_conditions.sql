...
begin
  ...
  if normalized_mode='customer_payment' and selected_rule_set_id is null then
    business_result:=public.apply_comp_business_condition_to_eligibility(selected_component_id,'customer_payment_received');
    if nullif(business_result->>'rule_set_id','') is not null then
      preview_result:=public.preview_comp_rule_set((business_result->>'rule_set_id')::uuid,25);
    end if;
    return business_result || jsonb_build_object('preview',preview_result,'previewed_automatically',true);
  end if;
